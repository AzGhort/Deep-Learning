...
class ResNet(tf.keras.Model):
    ...
    def __init__(self, args):
        ...
        conv1 = tf.keras.layers.Conv2D(filters=16,kernel_size=3,strides=1,padding='same',activation=tf.nn.relu)(inputs)
        # 32x32x32
        conv2 = self.add_convolution_group(5, 16, False, [3, 3], 'same', conv1)
        # 16x16x64
        conv3 = self.add_convolution_group(5, 32, True, [3, 3], 'same', conv2)
        # 8x8x128
        conv4 = self.add_convolution_group(5, 64, True, [3, 3], 'same', conv3)
    ...
    def add_residual_block(self, kernels, filt, padd, previous):
        ...
        for kernel in kernels:
            layer = self.add_bn_relu_conv(filt, kernel, padd, layer)
    ...
    def cutout_image_dataset(dataset, image_size, cutout_size, versions):
        ...
        for image in images:
            for _ in range(versions):
                ...
                new_im.shape = (1, image_size, image_size, 3)
                new_label.shape = (1, 1)
                images = np.append(images, new_im, axis=0)
...
if __name__ == "__main__":
    ...
    #DataAugment.cutout_image_dataset(cifar.train.data, 32, 16, 1)
    #DataAugment.cutout_image_dataset(cifar.test.data, 32, 16, 1)
    #DataAugment.cutout_image_dataset(cifar.dev.data, 32, 16, 1)
