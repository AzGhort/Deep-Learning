...
class Network:
    def __init__(self, args, num_words, num_tags, num_chars):
        ...
        embed_charseqs = tf.keras.layers.Embedding(num_chars, args.cle_dim, mask_zero=False)(charseqs)
        convs = []
        for width in range(2, args.cnn_max_width + 1):
            conv = tf.keras.layers.Conv1D(filters=args.cnn_filters, kernel_size=width,
                                          strides=1, padding="valid", activation=tf.nn.relu)(embed_charseqs)
            last = tf.keras.layers.GlobalMaxPooling1D()(conv)
            convs.append(last)
        concat1 = tf.keras.layers.Concatenate()(convs)
        dense1 = tf.keras.layers.Dense(args.we_dim, activation=tf.nn.relu)(concat1)

        embed_cle = tf.keras.layers.Lambda(lambda args: tf.gather(args[0], args[1]))([dense1, charseq_ids])

        embed_words = tf.keras.layers.Embedding(num_words, args.we_dim, mask_zero=True)(word_ids)
        concat2 = tf.keras.layers.Concatenate()([embed_words, embed_cle])
